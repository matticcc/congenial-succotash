...
def make_keyboard(data: dict, user_id: int) -> InlineKeyboardMarkup:
    """
    user_id is embedded in every callback_data — no in-memory map needed,
    survives bot restarts. Format: action###user_id
    """
    uid = str(user_id)
...
async def callback_query(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Parse owner_id from callback_data (format: action###user_id)
    try:
        action, uid_str = action.split("###", 1)
        owner_id = int(uid_str)
    except (ValueError, AttributeError):
        await query.answer("⚠️ Malformed callback data.", show_alert=True)
        return

    # Only the owner can use the controls
...
async def spotify_callback(request: web.Request) -> web.Response:
    ...
    try:
        await telegram_app.bot.send_message(
            ...
            text=(
                "✅ <b>Successfully connected to Spotify!</b>\n\n"
                "Now go to any chat, type <code>@currentspotifysongbot</code> "
                "and tap the result to share what you're listening to!"
            ),
            parse_mode=ParseMode.HTML,
        )
